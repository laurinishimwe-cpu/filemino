...
type DropZoneProps = { accept?: string; title: string; description?: string; actionLabel?: string; icon?: ReactNode; maxSize?: number; disabled?: boolean; selectedFile?: File | null; error?: string; onFileSelected: (file: File) => void; onRemove?: () => void; onChange?: () => void };
...
export function DropZone({ accept, title, description, actionLabel, icon, maxSize, disabled = false, selectedFile, error, onFileSelected, onRemove, onChange }: DropZoneProps) {
  ...
  if (selectedFile) return <div>{fileInput}<FilePreview file={selectedFile} icon={icon} onRemove={onRemove} onChange={onChange ?? (() => inputRef.current?.click())} disabled={disabled} /></div>;
