...
import { cancelJob, createImageConversionJob, FileMinoApiError, getDownload, initializeUpload, inspectImageUpload, uploadFile } from "@/lib/api/filemino-client";
...
const ACCEPTED_IMAGES = ".jpg,.jpeg,.png,.webp,.avif,.ico,.bmp,.tif,.tiff";

function isAbortError(error: unknown) { return error instanceof DOMException && error.name === "AbortError"; }
function messageForError(error: unknown) { return error instanceof FileMinoApiError ? errorMessageForImageCode(error.code) : "We couldn’t connect to FileMino. Please try again."; }

export function ImageConverter({ initialTarget, title = "Image Converter", description = "Convert images between popular formats.", supportingInfo = "PNG, JPG, WebP, AVIF, ICO and more" }: { initialTarget?: ImageConversionOutputFormat; title?: string; description?: string; supportingInfo?: string }) {
  ...
  const chooseFile = (next: File) => {
    ...
    const image = new window.Image(); image.onload = () => { if (previewRef.current === url) setHasAlpha(next.type === "image/png" || next.type === "image/webp" || next.type === "image/avif"); }; image.src = url;
  ...
  return <ToolPage title={title} description={description} badge="Image tool" supportingInfo={supportingInfo}>
    {(view === "idle" || view === "error" && !file) && <DropZone accept={ACCEPTED_IMAGES} title="Choose image" actionLabel="Choose image" icon={<FileToolIcon name="image" />} description="or drag and drop · PNG, JPG, WebP, AVIF, ICO, BMP, TIFF" onFileSelected={chooseFile} error={error ?? undefined} />}
    {(view === "selected" || view === "error" && file) && file && <div className="tool-selection-flow">
      ...
      {capability ? <Card className="tool-controls"><div className="tool-control-heading"><div><h2>Convert to</h2><p className="tool-control-description">FileMino will verify the uploaded format before converting.</p></div></div><div className="image-format-options" role="radiogroup" aria-label="Output format">{capability.targets.map((value) => <button type="button" key={value} role="radio" aria-checked={target === value} className={`image-format-option ${target === value ? "is-selected" : ""}`} onClick={() => setTarget(value)}><strong>{imageConversionTargetDetails[value].label}</strong><span>{imageConversionTargetDetails[value].description}</span>{capability.recommended === value && <em>Recommended</em>}</button>)}</div>
        ...
        <div className="tool-cta"><Button onClick={() => void convert()}>Convert Image</Button></div></Card> : <ToolErrorCard title="Unsupported image" message="Choose a PNG, JPG, WebP, AVIF, ICO, BMP, or TIFF image." onRetry={reset} onReset={reset} resetLabel="Choose another image" />}
