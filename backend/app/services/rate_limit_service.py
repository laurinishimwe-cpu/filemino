...
class RateLimitService:
    ...
    def consume_job_request(self,subject:str,limit:int)->None:
        key=f"filemino:rate:jobs:{subject}:{int(time.time()//3600)}"; count=self.redis.incr(key)
    ...
    def claim_concurrent(self,subject:str,job_id:UUID,limit:int,ttl:int)->None:
        key=f"filemino:active:{subject}"; now=time.time(); result=self.redis.eval("redis.call('ZREMRANGEBYSCORE',KEYS[1],'-inf',ARGV[1]);if redis.call('ZCARD',KEYS[1])>=tonumber(ARGV[2]) then return 0 end;redis.call('ZADD',KEYS[1],ARGV[3],ARGV[4]);redis.call('EXPIRE',KEYS[1],ARGV[5]);return 1",1,key,now,limit,now+ttl,str(job_id),ttl)
        if result!=1:raise TooManyActiveJobsError()
    def release_concurrent(self,subject:str,job_id:UUID)->None:self.redis.zrem(f"filemino:active:{subject}",str(job_id))
